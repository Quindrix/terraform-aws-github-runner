...
| <a name="input_subnet_ids"></a> [subnet\_ids](#input\_subnet\_ids) | List of subnets in which the action runner instances will be launched. The subnets must exist in the configured VPC (`vpc_id`). Multiple subnets in the same Availability Zone are supported; each EC2 Fleet request uses at most one subnet per Availability Zone. | `list(string)` | n/a | yes |
